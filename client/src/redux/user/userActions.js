...
import { generatedAccount, accountEncrypted } from '../../wallet/wallet';
...
export const signUpStart = (email, fullName, username, password) => async (dispatch) => {
  try {
    dispatch({ type: userTypes.SIGN_IN_START });
    //// set ethAccount here..?
    const ethAccount = generatedAccount
    const ethAccPublic = ethAccount.address
    const ethAccPrivate = accountEncrypted(ethAccount, password)
    const response = await registerUser(email, fullName, username, password, ethAccPublic, ethAccPrivate);
