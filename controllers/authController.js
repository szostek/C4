  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const user = await User.findOne(
        { _id: id },
        'email username avatar bookmarks bio fullName confirmed website ethAccPublic ethAccPrivate'
      );
      if (user) {
        return resolve(user);
      } else {
...
module.exports.register = async (req, res, next) => {
  ...
  const { username, fullName, email, password, ethAccPublic, ethAccPrivate } = req.body;
  let user = null;
  ...
  try {
    user = new User({ username, fullName, email, password, ethAccPublic, ethAccPrivate });
